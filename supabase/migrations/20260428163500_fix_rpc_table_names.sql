-- CORRIGE NOMES DAS TABELAS NA FUNCAO RPC
...
-- ATUALIZA FUNCAO DE TENANTS COM NOMES CORRETOS DAS TABELAS
...
BEGIN
    ...
    SELECT 
        ...
        ap.plan_id,
        p.name as plan_name,
        COUNT(uf.id) as forms_count,
        COUNT(fr.id) as submissions_count,
        ap.last_login
    FROM admin_profile ap
    LEFT JOIN plans p ON p.id = ap.plan_id
    LEFT JOIN forms uf ON uf.user_id = ap.user_id
    LEFT JOIN form_submissions fr ON fr.form_id = uf.id
